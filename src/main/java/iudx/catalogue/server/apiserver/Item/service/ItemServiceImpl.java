...
public class ItemServiceImpl implements ItemService {
  ...
  public ItemServiceImpl(ElasticsearchService esService,
                         GeocodingService geoService,
                         NLPSearchService nlpService, JsonObject config) {
  ...
  private static QueryModel checkQueryModel(String id) {
    ...
    Map<String, Object> resourceGroupParams =
        Map.of(FIELD, RESOURCE_GRP + KEYWORD_KEY, VALUE, id);
    ...
    List<QueryModel> shouldQueries =
        List.of(idQuery, resourceGroupQuery, providerQuery, resourceServerQuery,
            cosQuery);
    ...
    verifyInstance(instanceId)
        ...
        .onFailure(err -> {
          if ("Item already exists".equals(err.getMessage())) {
            ...
            promise.fail(
                operationNotAllowedResponse(doc.getString(ID), INSERT, err.getLocalizedMessage()));
          }
        });
  ...
  public Future<JsonObject> updateItem(Item item) {
    ...
    new Timer()
        .schedule(
            new TimerTask() {
              public void run() {
                esService.search(index, queryModel).onComplete(
                    checkRes -> {
                      if (checkRes.failed()) {
                      ...
                      if (checkRes.succeeded()) {
                        ...
                        esService.updateDocument(index, docId, doc)
                            .onComplete(dbHandler -> {
                              if (dbHandler.failed()) {
                                LOGGER.error(
                                    "Fail: Item update failed; " + dbHandler.cause().getMessage());
                                promise.fail(internalErrorResp());
                              } else {
                                LOGGER.info("Success: Item updated;");
                                promise.complete(successfulItemOperationResp(doc,
        ...
        .schedule(
            new TimerTask() {
              public void run() {
                ...
                esService.search(index, queryResourceGrp)
                    .onComplete(checkRes -> {
                      if (checkRes.failed()) {
                        ...
                        if (checkRes.result().size() > 1) {
                          ...
                          promise.fail(operationNotAllowedResponse(id,
                            ...
                            .onComplete(dbHandler -> {
                              if (dbHandler.succeeded()) {
                                ...
                                if (TITLE_SUCCESS.equals(response.getString(TITLE))) {
                                  promise.complete(
                                      successResp(id, "Success: Item deleted successfully"));
                                } else {
                  ...
                  if (ar.succeeded()) {
                    ...
                    new Timer()
                        .schedule(
                            new TimerTask() {
                              public void run() {
                                esService.createDocument(config.getString(DOC_INDEX), doc)
                                    .onComplete(dbHandler -> {
                                      if (dbHandler.failed()) {
                                        LOGGER.error("Fail: Item creation failed; " +
