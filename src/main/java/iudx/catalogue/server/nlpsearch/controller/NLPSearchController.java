    ...
    void handleSearch(RoutingContext routingContext) {
        ...
        if (query == null || query.isEmpty()) {
            routingContext.response()
                    .setStatusCode(400)
                    .end(new RespBuilder().withType(TYPE_INVALID_SYNTAX).withTitle(TITLE_INVALID_SYNTAX).getResponse());
            return;
        }

        nlpSearchService.search(query)
                .onSuccess(result -> {
                ...
                .onFailure(err -> {
                    ...
                    routingContext.response()
                            .setStatusCode(500)
                            .end(new RespBuilder().withType(TYPE_INTERNAL_SERVER_ERROR).withTitle(TITLE_INTERNAL_SERVER_ERROR).getResponse());
                });
    }

    void handleGetEmbedding(RoutingContext routingContext) {
        routingContext.request().body().onSuccess(buffer -> {
            JsonObject doc = buffer.toJsonObject();
            nlpSearchService.getEmbedding(doc)
                    .onSuccess(result -> {
                        routingContext.response()
                                .setStatusCode(200)
                                .putHeader("content-type", "application/json")
