...
public class MlayerPopularDatasets {
  ...
  public MlayerPopularDatasets(WebClient webClient,
                               ElasticsearchService esService, String docIndex,
                               String mlayerInstanceIndex,
                               String mlayerDomainIndex) {
  ...
  private void allMlayerDomains(Promise<JsonArray> domainResult) {
    ...
    esService.search(mlayerDomainIndex, getAllMlayerDomainQueryModel)
        .onComplete(getDomainHandler -> {
  ...
  private void datasets(
    ...
    resourceGroupQuery.addMustQuery(new QueryModel(QueryType.MATCH, Map.of(FIELD,
        "instance.keyword", VALUE, instance)));
    ...
    esService.search(docIndex, providerAndResourcesQueryModel)
        .onComplete(getCatRecords -> {
    ...
    esService.search(docIndex, finalQueryModel)
        .onComplete(latestRgHandler -> {
          if (latestRgHandler.succeeded()) {
            ...
            JsonArray latestDatasets = responseMsg.getResponse().getJsonArray(RESULTS);
            int resourceGroupCount =
                responseMsg.getResponse().getJsonObject("count").getInteger("resourceGroupCount");

            int resourcesCount =
                responseMsg.getResponse().getJsonObject("count").getInteger("resourceCount");

            int providerCount =
                responseMsg.getResponse().getJsonObject("count").getInteger("providerCount");

            QueryModel termsQuery = new QueryModel(QueryType.TERMS, Map.of(
                FIELD, ID_KEYWORD, VALUE, frequentlyUsedResourceGroup.getList()
            ));
            QueryModel termQuery = new QueryModel(QueryType.TERM, Map.of(
                FIELD, "type.keyword", VALUE, "iudx:Provider"
            ));
            ...
            providerAndPopularRgs.setIncludeFields(List.of("id", "description", "type",
                "resourceGroup", "accessPolicy", "provider", "itemCreatedAt", "instance", "label"));
            providerAndPopularRgs.setLimit(MAX_LIMIT);

            esService.search(docIndex, providerAndPopularRgs)
                .onComplete(providerAndPopularRgHandler -> {
                  if (providerAndPopularRgHandler.succeeded()) {
                    List<ElasticsearchResponse> respList = providerAndPopularRgHandler.result();
